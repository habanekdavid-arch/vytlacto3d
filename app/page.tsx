            ...
            <div className="flex-1 overflow-y-auto">
              <div className="p-5 space-y-5">
                ...
                <CartSidebar
                  ...
                  onRemove={removeItem}
                  onAddNew={() => { uploadBoxRef.current?.triggerOpen(); setActiveItemId(null); closeCart(); }}
                />
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {cartItems.map((item, idx) => (
                  <button
                    ...
                    className={[
                      "group flex-none flex items-center gap-2 rounded-2xl border-2 px-3 py-2 text-left transition-all",
                  ...
                  >
                    <span className={[
                      ...
                      activeItemId === item.id ? "bg-[#FFAE00] text-black" : "bg-neutral-200 text-neutral-600",
                    ].join(" ")}>{idx + 1}</span>
                    <div className="min-w-0">
                      <div className="max-w-[120px] truncate text-xs font-semibold text-neutral-900">{item.fileName}</div>
                      <div className="text-[10px] text-neutral-500">
                        {item.config.material}{item.pricing ? ` · ${formatEur(addVat(item.pricing.total))}` : " · …"}
                      </div>
                    </div>
                    <span
                      role="button"
                      onClick={(e) => { e.stopPropagation(); removeItem(item.id); }}
                      className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-neutral-300 transition hover:bg-red-50 hover:text-red-500"
                    >
                ...
                <button
                  type="button"
                  onClick={() => { uploadBoxRef.current?.triggerOpen(); setActiveItemId(null); }}
                  className="flex-none flex items-center gap-1.5 rounded-2xl border-2 border-dashed border-neutral-300 bg-neutral-50 px-3 py-2 text-xs font-semibold text-neutral-500 transition hover:border-[#FFAE00] hover:bg-[#FFAE00]/5 hover:text-neutral-900"
                >
                ...
                <div className="flex gap-3">
                  ...
                  {cartItems.length > 0 && (
                    <button
                      type="button"
                      onClick={openCart}
                      className="flex flex-[7] items-center justify-center gap-2 rounded-2xl bg-[#FFAE00] px-6 py-4 text-base font-extrabold text-black shadow-sm transition hover:bg-[#e09d00]"
                    >
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
                        <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
